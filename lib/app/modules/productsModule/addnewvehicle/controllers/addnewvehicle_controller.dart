...
import 'package:auction_app/app/repositories/additemsRepository/addItems_repository.dart';
...
class AddnewvehicleController extends GetxController {
  ...
  Future<void> addNewProduct() async {
    try {
      ...
      if (bodyTypes.contains(selectedBodyType.value) == false) {
        await addNewBodyType(selectedBodyType.value);
      }
      if (transmissionTypes.contains(selectedTransmission.value) == false) {
        await addNewTransmission(selectedTransmission.value);
      }
      if (colors.contains(selectedColor.value) == false) {
        await addNewColor(selectedColor.value);
      }
      if (displacements.contains(selectedDisplacement.value) == false) {
        await addNewDisplacement(selectedDisplacement.value);
      }
      if (driverTypes.contains(selectedDriverType.value) == false) {
        await addNewDriveType(selectedDriverType.value);
      }
      if (scoreValues.contains(selectedScore.value) == false) {
        await addNewScore(selectedScore.value);
      }
...
// Add this helper method for bike features
  ...
  AddItemsRepository addnewItemsRepository = AddItemsRepository();

  Future addNewBodyType(String bodyType) async {
    try {
      print("body type here");
      return await addnewItemsRepository.addNewBodyType(bodyType);
    } catch (e) {
  ...
  Future addNewTransmission(String transmission) async {
    try {
      return await addnewItemsRepository.addNewTransmission(transmission);
    } catch (e) {
  ...
  Future addNewColor(String color) async {
    try {
      return await addnewItemsRepository.addNewColor(color);
    } catch (e) {
  ...
  Future addNewDisplacement(String displacement) async {
    try {
      await addnewItemsRepository.addNewDisplacement(displacement);
    } catch (e) {
  ...
  Future addNewDriveType(String driveType) async {
    try {
      print("driver type here");

      return await addnewItemsRepository.addNewDriveType(driveType);
    } catch (e) {
  ...
  Future addNewScore(String score) async {
    try {
      print("score type here");

      return await addnewItemsRepository.addNewScore(score);
    } catch (e) {
