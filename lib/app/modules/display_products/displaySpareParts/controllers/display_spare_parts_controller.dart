...
import '../../../../data/getModels/get_all_spareparts_data_model.dart';
import '../../../../repositories/products_repository/products_rep.dart';
import '../../../modules.dart';

class DisplaySparePartsController extends GetxController {
  final FocusNode searchFocusNode = FocusNode();

  final data = <SparePartData>[].obs;

  // Paginated data for the current page
  final paginatedData = <SparePartData>[].obs;

  // Filtered data based on status
  final filteredData = <SparePartData>[].obs;
    ...
    if (query.isEmpty) {
      ...
      filteredData.assignAll(
        data.where((part) =>
            ...
            part.name.toString().contains(query) ||
            part.model.toString().toLowerCase().contains(query.toLowerCase())),
      );
  ...
  void onInit() async {
    super.onInit();
    await getAllRegisteredSpareParts();
    paginateData();
  }

  // Fetching all vehicles
  Future<void> getAllRegisteredSpareParts() async {
    try {
      isLoading.value = true;
      final response = await productsRepository.getAllSparePartsToDisplay();
