...
class DisplayTrucksController extends GetxController {
  final FocusNode searchFocusNode = FocusNode();

  final data = <GetIndividualTrucksData>[].obs;

  // Paginated data for the current page
  final paginatedData = <GetIndividualTrucksData>[].obs;

  // Filtered data based on status
  final filteredData = <GetIndividualTrucksData>[].obs;
    ...
    if (query.isEmpty) {
      ...
      filteredData.assignAll(
        data.where((truck) =>
            ...
            truck.name.toString().contains(query) ||
            truck.model.toString().toLowerCase().contains(query.toLowerCase())),
      );
  ...
  void onInit() async {
    super.onInit();
    await getAllRegisteredTrucks();
    paginateData();
  }

  // Fetching all vehicles
  Future<void> getAllRegisteredTrucks() async {
    try {
      isLoading.value = true;
      final response = await productsRepository.getAllTrucksToDisplay();
